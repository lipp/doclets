...
var createTaffyData = function (jsFiles, options) {
	...
	var taffyData = taffy(JSON.parse(fs.readFileSync(taffyJson)));
	fse.copySync(taffyJson, path.join(options.outDir, 'taffy.json'));
...
var generateDoc = function (files, options, rawData) {
	var t1 = new Date();
	var taffyData = createTaffyData(files, options);
	var t2 = new Date();
	rawData.time = {};
	rawData.time.create = t2 - t1;
	t1 = t2;
	var modules = utils.buildHierarchy(taffyData, options);
	t2 = new Date();
	rawData.time.process = t2 - t1;
	t1 = t2;

	rawData.taffy = JSON.parse(fs.readFileSync(path.join(options.outDir, 'taffy.json')));
	...
	rawData.time.render = new Date() - t1;
...
var generateGitDoc = function (gitDir, repoUrl, versionString, htmlDir) {
	...
	fs.writeFileSync(path.join(outDir, 'raw.json'), JSON.stringify(rawData, null, '\t'));
	return outDir;
};
