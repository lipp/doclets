...
var tools = module.exports.tools = {
  ...
  splitName: function (doclet) {
    ...
    if (match) {
      var splitIndex = match[3].lastIndexOf('.')
      if (splitIndex < 0) {
        splitIndex = match[3].lastIndexOf('#')
      }
      if (splitIndex < 0) {
        splitIndex = match[3].lastIndexOf('~')
      }
      return {
        module: match[1],
        name: splitIndex > -1 ? match[3].substr(splitIndex + 1) : match[3],
        parent: splitIndex > -1 ? match[3].substr(0, splitIndex + 1) : ''
      }
    } else {
      return {
        module: '',
