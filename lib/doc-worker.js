    ...
    .exec(function (err, repos) {
      if (err) {
        ...
        async.parallel(refireRepoEventsFns, function (err) {
          if (err) {
            console.log('catchup failed during refiring events', err)
          } else {
            console.log('catchup success')
          }
          done(err)
        })
      }
    ...
    User.findById(username, function (err, user) {
      try {
        ...
        if (!data.ignored) {
          ...
          console.log('ignored doc', repository.html_url, branch)
          done()
        }
      } catch (err) {
        done(err)
    ...
    try {
      ...
      var done = function (err, result) {
        if (err) {
          ...
          console.log('job finished', data.repository.full_name, data.ref)
          jobDone()
        }
      }
