...
var init = function (port, initDone) {
  ...
  app.use(function (req, res, next) {
    ...
    function lookupSession (error) {
      ...
      if (tries < 0) {
        console.log('Session retry failed')
        return next()
      }
      if (tries !== 9) {
        console.log('Session retry', tries)
      }
      setTimeout(function () {
  ...
  router.post('/account/changerepo', ensureAuthenticated, routes.changeRepo.bind(routes))
  router.post('/sync', ensureAuthenticated, routes.sync.bind(routes))
  router.post('/reauth', ensureAuthenticated, routes.reauth.bind(routes))

  router.get('/', routes.index.bind(routes))

  router.get('/users', routes.users.bind(routes))
  router.get('/browse', routes.doclets.bind(routes))
  router.get('/search', routes.search.bind(routes))
  router.get('/doclets.yml', routes.docletsYml.bind(routes))

  router.get('/:user', routes.user.bind(routes))

  router.get('/:user/:repo', routes.repo.bind(routes))

  router.get('/:user/:repo/:version.svg', routes.badge.bind(routes))
  router.get('/:user/:repo/:version', routes.api.bind(routes))

  router.get('/:user/:repo/:version/:article', routes.article.bind(routes))
  router.get('/:user/:repo/:version/g/:group', routes.api.bind(routes))
