...
var docletSchema = new Schema({
  ...
  dataJson: String // the real doc data
})
...
docletSchema.methods.getGitHubRepoUrl = function () {
  return 'https://github.com/' + this.owner + '/' + this.repo + '/tree/' + this.tagOrHash
}
...
docletSchema.methods.asFlat = function (group, onlyLeafs) {
  console.log('asFlat', group, onlyLeafs)
  return structure.flat(this.data.doclets, group, onlyLeafs)
}
...
docletSchema.methods.hasUserAccess = function (user) {
  return !!(this.isPublic || (user && user.accessibleRepos && user.accessibleRepos.indexOf(this.owner + '/' + this.repo) > -1))
}

docletSchema.statics.findByFullnames = function (fullNames, done) {
  var getDoclets = fullNames.map(function (fullName) {
    var parts = fullName.split('/')
    return function (callback) {
      Doclet.find({owner: parts[0], repo: parts[1]}, callback)
    }
  })
  async.parallel(getDoclets, function (err, results) {
    if (err) {
      done(err)
      return
    }
