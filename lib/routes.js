...
Routes.prototype.badge = function (req, res) {
  ...
  Doclet.findById(key, function (err, doclet) {
    ...
    if (doclet.error) {
      res.redirect('https://img.shields.io/badge/API--Doc_' + req.params.version + '-failed-red.svg')
      return
    } else {
      res.redirect('https://img.shields.io/badge/API--Doc_' + req.params.version + '-ready-green.svg')
      return
    }
  })
}
...
Routes.prototype.addRepo = function (req, res) {
  ...
  var done = function (err) {
    if (err) {
      ...
      req.flash('result', 'Added repository ' + fullname + ' to Doclets.io')
      res.redirect('/' + req.params.user)
    }
  }
...
Routes.prototype.changeRepo = function (req, res) {
  async.parallel([
    ...
    if (err) {
      ...
      actions.push(repo.updateWebHook.bind(repo, req.body))
