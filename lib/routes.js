...
Routes.prototype.user = function (req, res) {
  User.findById(req.params.user, function (err, user) {
    ...
    async.series([
      function (callback) {
        if ((user.type === 'Organization' && user.token) || (req.user && req.user._id === req.params.user)) {
...
Routes.cache = {}
Routes.renderApi = jade.compileFile('./views/api.jade')

Routes.isCached = function (req, doclet) {
  return Routes.cache[req.path] && Routes.cache[req.path].date.toString() === doclet.createdAt.toString()
}
...
Routes.prototype.api = function (req, res) {
  ...
  Doclet.findById(key, function (err, doclet) {
    ...
    if (!doclet.hasUserAccess(req.user)) {
      res.status(404).send('Not public')
      return
    }
    if (Routes.isCached(req, doclet)) {
      console.log('from cache')
      res.send(Routes.cache[req.path].html)
      return
    }
    doclet.populate(['_repo', '_owner'], function (err, doclet) {
      if (err || !doclet) {
        res.status(500).send('Err ' + err)
        return
      }
      ...
      params.group = (req.params.group === '__global') ? '' : req.params.group
      Routes.cache[req.path] = {
        html: Routes.renderApi(params),
        date: doclet.createdAt
      }
      res.send(Routes.cache[req.path].html)
    })
  })
...
Routes.renderArticle = jade.compileFile('./views/article.jade')
...
Routes.prototype.article = function (req, res) {
  ...
  Doclet.findById(key, function (err, doclet) {
    ...
    doclet.populate(['_repo', '_owner'], function (err, doclet) {
      ...
      params.cachable = true
      Routes.cache[req.path] = {
        html: Routes.renderArticle(params),
        date: doclet.createdAt
      }
      res.send(Routes.cache[req.path].html)
    })
