...
Routes.prototype.account = function (req, res) {
  async.parallel([
    ...
    this.db.getModulesByUser.bind(this.db, req.user.profile.username)
...
Routes.prototype.user = function (req, res) {
  async.parallel([
    this.db.getModulesByUser.bind(this.db, req.params.user),
    this.db.getUserByUsername.bind(this.db, req.params.user)
  ], function (err, results) {
    if (err) {
      res.status(500).send('Err ' + err)
      return
