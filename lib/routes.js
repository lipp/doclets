...
Routes.prototype.changeRepo = function (req, res) {
	async.parallel([
		Doclet.find.bind(Doclet, {owner: req.user._id, repo: req.params.repo}),
	 	Repo.findById.bind(Repo, req.user._id + '/' + req.params.repo)
		...
		if (err) {
			...
			var actions = _.chain(doclets)
				.filter(function(doclet) {
					var key = '_public-' + doclet.version
					console.log(doclet.isPublic, req.body[key])
					if (!doclet.isPublic && req.body[key] !== 'on') {
							return true
					}
					if (doclet.isPublic && req.body[key] === undefined) {
							return true
					}
					return false
				})
				.map(function(doclet) {
								return function(callback) {
												console.log('toggling', doclet._id, doclet.version)
												doclet.isPublic = !doclet.isPublic
												doclet.save(callback)
								}
				})
				.value()

				if (req.body._enabled === 'on' && !repo.isWebHookEnabled()) {
					actions.push(function(callback) {
									repo.enableWebHook(callback)
					})
				} else if (req.body._enabled !== 'on' && repo.isWebHookEnabled()) {
					actions.push(function(callback) {
									repo.disableWebHook(callback)
					})
				}
			async.parallel(actions, function(err) {
				if (err) {
  				...
  				res.redirect('/' + req.user._id + '/' + req.params.repo)
				}
			})
		}
	})
...
Routes.prototype.sync = function (req, res) {
  console.log('syncing user', req.user._id)
  Repo.sync(req.user._id, {type: 'oauth', token: req.user.token}, function (err) {
    if (err) {
      res.status(500).send('Err ' + err)
    } else {
...
Routes.prototype.user = function (req, res) {
  async.parallel([
    Doclet.findByOwner.bind(Doclet, req.params.user),
    User.findById.bind(User, req.params.user),
    function (callback) {
      if (req.user) {
        Repo.findOrSyncByUser(req.user._id, {type: 'oauth', token: req.user.token}, callback)
      } else {
