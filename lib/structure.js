...
var replaceInlineLinks = module.exports.replaceInlineLink = function (str) {
  str = str.replace(/\[(.+?)\]\s*{@link(?:plain|code)? (.+?)}/g, function (match, text, link) {
    return createLink(link, text)
  })
  str = str.replace(/{@link(?:plain|code)? ([^\s]+?)[\s|\|](.+?)}/g, function (match, link, text) {
    return createLink(link, text)
  })
  str = str.replace(/{@link(?:plain|code)? ([^\s]+?)}/g, function (match, link) {
