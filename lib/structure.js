...
var getModuleName = function (doclet) {
  return doclet.longname.match(/module:[^~\.#]+/)
}
...
var getGroup = function (namespaces) {
  return function (doclet) {
    return {
      category: getCategory(doclet),
      module: getModuleName(doclet),
      namespace: getNamespace(doclet, namespaces)
    }
  }
}

var getGroups = function (doclets) {
  var namespaces = getNamespaces(doclets)
  var groupsById = doclets
    .map(getGroup(namespaces))
    .reduce(function (groups, group) {
      var id = (group.module || '') + (group.namespace || '') + (group.category || '')
      group.id = id
      groups[id] = group
      return groups
    }, {})
