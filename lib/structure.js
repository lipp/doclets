...
var addUrlToDoclets = function (doclets, dir, fileBaseUrl) {
	_.each(doclets, function (doclet) {
		try {
			var rel = path.join(doclet.meta.path.split(dir)[1] || '', doclet.meta.filename);
...
module.exports.buildHierarchy = function (data, skipUrl) {
	...
	if (!skipUrl) {
		addUrlToDoclets(doclets, data.genDir, fileRootFromGitHubUrl(data.githubUrl, data.branch));
	}
