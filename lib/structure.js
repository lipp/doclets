...
var getLink = function (typeName) {
  var bi = builtInTypes[typeName.toLowerCase()]
  if (bi) {
    return bi
  } else {
    return '#' + idFromLongname(typeName)
  }
}

var getClosureParams = module.exports.getClosureParams = function (param) {
...
var addTypeLinks = module.exports.addTypeLinks = function (doclet) {
  var fields = ['params', 'returns']
  fields.forEach(function (field) {
    if (doclet[field]) {
      doclet[field].forEach(function (param) {
        if (param.type && param.type.names) {
          param.type.typeNames = param.type.names.map(getClosureParams)
        } else {
  ...
  if (doclet.type && doclet.type.names) {
    doclet.type.typeNames = doclet.type.names.map(getClosureParams)
  }
}
