...
var createDoclets = function (config, workDir) {
  ...
  jsdocOptions.push('-t')
  jsdocOptions.push(path.join(__dirname, '/capture-template'))
  jsdocOptions.push('-c')
  jsdocOptions.push(path.join(__dirname, '/jsdoc.conf'))
  var filename = uuid.v1()
  jsdocOptions.push('-d')
  jsdocOptions.push(filename)
  console.log('jsdoc starting')
  var jsdoc = exec(path.join(__dirname, '/../node_modules/.bin/jsdoc ') + jsdocOptions.join(' '), {
    cwd: __dirname
  })
